...
// Set — набор endpoint’ов сервиса
type Set struct {
	AggregateEndpoint endpoint.Endpoint
	CalculateEndpoint endpoint.Endpoint
}

// New — собирает endpoint’ы с middleware
func New(svc aggservice.Service, logger log.Logger) Set {
	...
	var aggregateEndpoint endpoint.Endpoint
	{
		...
		// rate limit: 1 запрос в секунду, burst = 1
...
// Calculate — удобный метод-обёртка над endpoint
...
// Aggregate — удобный метод-обёртка над endpoint
...
// MakeAggregateEndpoint — адаптер service → endpoint
...
// MakeConcatEndpoint — адаптер service → endpoint (Calculate)
